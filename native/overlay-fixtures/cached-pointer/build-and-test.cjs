...
const build = () => {
  assert.equal(process.platform, "win32", "cached-pointer QA is Windows-only");
  verifyPinnedDetours();
  ...
  const targetObject = path.join(objectRoot, "cached-pointer-target.obj");
  const targetDll = path.join(buildRoot, "gamehub-cached-pointer-target64.dll");
  const targetLibrary = path.join(
    buildRoot,
    "gamehub-cached-pointer-target64.lib"
  );
  compileObject(
    vcvars,
    path.join(fixtureRoot, "target.cpp"),
    targetObject,
    [fixtureRoot],
    ["/DGAMEHUB_CACHED_POINTER_TARGET_EXPORTS"]
  );
  linkDll(
    vcvars,
    targetDll,
    targetLibrary,
    path.join(fixtureRoot, "target.def"),
    [targetObject],
    ["kernel32.lib"]
  );

  const cacheObject = path.join(objectRoot, "cached-pointer-cache.obj");
  const cacheDll = path.join(buildRoot, "gamehub-cached-pointer-cache64.dll");
  const cacheLibrary = path.join(
    buildRoot,
    "gamehub-cached-pointer-cache64.lib"
  );
  compileObject(
    vcvars,
    ...
    cacheObject,
    [fixtureRoot],
    ["/DGAMEHUB_CACHED_POINTER_CACHE_EXPORTS"]
  );
  linkDll(
    vcvars,
    cacheDll,
    ...
    [cacheObject],
    [targetLibrary, "kernel32.lib"]
  );

  const bootstrapObject = path.join(objectRoot, "cached-pointer-bootstrap.obj");
  const bootstrapDll = path.join(
    buildRoot,
    "gamehub-overlay-qa-cached-pointer-bootstrap64.dll"
  );
  const bootstrapLibrary = path.join(
    buildRoot,
    "gamehub-overlay-qa-cached-pointer-bootstrap64.lib"
  );
  compileObject(
    vcvars,
  ...
  linkDll(
    ...
    [bootstrapObject],
    [cacheLibrary, targetLibrary, detoursLibrary, "kernel32.lib"]
  );
  const cacheImports = captureMsvc(vcvars, "dumpbin.exe", [
    "/imports",
    cacheDll,
  ]);
  assert.match(cacheImports, /gamehub-cached-pointer-target64\.dll/iu);
  assert.match(cacheImports, /GameHubCachedPointerTarget/u);
  const bootstrapImports = captureMsvc(vcvars, "dumpbin.exe", [
    "/imports",
    bootstrapDll,
  ]);
  assert.match(bootstrapImports, /gamehub-cached-pointer-cache64\.dll/iu);
  assert.match(bootstrapImports, /GameHubCachedPointerCacheAddress/u);
  assert.match(bootstrapImports, /gamehub-cached-pointer-target64\.dll/iu);
  assert.match(bootstrapImports, /GameHubCachedPointerTarget/u);

  const fixtureObject = path.join(objectRoot, "cached-pointer-fixture.obj");
  ...
  runMsvc(vcvars, "link.exe", [
    ...
    "/NODEFAULTLIB",
    "/INCREMENTAL:NO",
    "/ENTRY:GameHubCachedPointerFixtureEntry",
  ...
  const launcherObject = path.join(objectRoot, "cached-pointer-launcher.obj");
