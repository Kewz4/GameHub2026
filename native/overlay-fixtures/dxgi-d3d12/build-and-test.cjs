...
const reuseDetours = process.env.GAMEHUB_DXGI_D3D12_QA_REUSE_DETOURS === "1";
...
const verifyPinnedDetours = () => {
  ...
  assert.deepEqual(manifestPaths, expectedDetoursManifestPaths);
};
...
const assertHardenedX64 = (vcvars, binary) => {
  const headers = runMsvc(vcvars, "dumpbin.exe", ["/headers", binary], true);
  assert.match(headers, /^\s+8664 machine \(x64\)\s*$/mu);
  assert.match(headers, /high entropy virtual addresses/iu);
  assert.match(headers, /dynamic base/iu);
  assert.match(headers, /nx compatible/iu);
  assert.match(headers, /control flow guard/iu);
  const loadConfig = runMsvc(
    vcvars,
    "dumpbin.exe",
    ["/loadconfig", binary],
    true
  );
  assert.match(loadConfig, /guard flags/iu);
  assert.match(loadConfig, /cf instrumented/iu);
};

const build = () => {
  assert.equal(process.platform, "win32", "DXGI/D3D12 QA is Windows-only");
  verifyPinnedDetours();
  ...
  const fixtureObject = path.join(objectRoot, "dxgi-d3d12-fixture.obj");
  ...
  const bootstrapObject = path.join(objectRoot, "dxgi-d3d12-bootstrap.obj");
  const bootstrapDll = path.join(
    buildRoot,
    "gamehub-overlay-qa-dxgi-d3d12-bootstrap64.dll"
  );
  const bootstrapLib = path.join(
    buildRoot,
    "gamehub-overlay-qa-dxgi-d3d12-bootstrap64.lib"
  );
  ...
  const launcherObject = path.join(objectRoot, "dxgi-d3d12-launcher.obj");
  ...
  [fixtureExe, bootstrapDll, launcherExe].forEach((binary) =>
    assertHardenedX64(vcvars, binary)
  );
