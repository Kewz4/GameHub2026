...
const parseDumpbinExports = (output) => {
  ...
  for (const line of output.split(/\r?\n/u)) {
    ...
    if (named) exports.push({ ordinal: Number(named[1]), name: named[2] });
  }
  return exports.sort((left, right) => left.ordinal - right.ordinal);
};

const build = () => {
  assert.equal(process.platform, "win32", "synthetic WGI QA is Windows-only");
  verifyPinnedDetours();
  ...
  const providerObject = path.join(objectRoot, "wgi-provider.obj");
  const providerDll = path.join(buildRoot, "gamehub-wgi-qa-provider64.dll");
  const providerLibrary = path.join(buildRoot, "gamehub-wgi-qa-provider64.lib");
  compileObject(
    vcvars,
  ...
  linkDll(
    ...
    [providerObject],
    ["kernel32.lib", "runtimeobject.lib"]
  );
  ...
  assert.deepEqual(parseDumpbinExports(providerExports), [
    { ordinal: 1, name: "RoGetActivationFactory" },
    { ordinal: 1001, name: "GameHubSyntheticWgiGetTopology" },
    { ordinal: 1002, name: "GameHubSyntheticWgiGetProviderSnapshot" },
    { ordinal: 1003, name: "GameHubSyntheticWgiSetFault" },
    { ordinal: 1004, name: "GameHubSyntheticWgiMutateTopology" },
    { ordinal: 1005, name: "GameHubSyntheticWgiSetNeutral" },
    { ordinal: 1006, name: "GameHubSyntheticWgiExerciseOverRelease" },
  ]);

  const cacheObject = path.join(objectRoot, "wgi-cache.obj");
  const cacheDll = path.join(buildRoot, "gamehub-wgi-qa-cache64.dll");
  const cacheLibrary = path.join(buildRoot, "gamehub-wgi-qa-cache64.lib");
  compileObject(
    vcvars,
  ...
  linkDll(
    ...
    [cacheObject],
    [providerLibrary, "kernel32.lib", "runtimeobject.lib"]
  );
  const cacheImports = captureMsvc(vcvars, "dumpbin.exe", [
    "/imports",
    cacheDll,
  ]);
  assert.match(cacheImports, /gamehub-wgi-qa-provider64\.dll/iu);
  for (const ordinal of [1001, 1002, 1003, 1004, 1005, 1006]) {
    assert.match(
      cacheImports,
  ...
  const bootstrapObject = path.join(objectRoot, "wgi-bootstrap.obj");
  const bootstrapDll = path.join(
    buildRoot,
    "gamehub-overlay-qa-wgi-bootstrap64.dll"
  );
  const bootstrapLibrary = path.join(
    buildRoot,
    "gamehub-overlay-qa-wgi-bootstrap64.lib"
  );
  compileObject(
    vcvars,
  ...
  assert.match(bootstrapImports, /gamehub-wgi-qa-cache64\.dll/iu);
  assert.match(bootstrapImports, /GameHubWgiQaGetCacheSnapshot/u);

  const fixtureObject = path.join(objectRoot, "wgi-fixture.obj");
  const fixtureExecutable = path.join(
    buildRoot,
    "gamehub-overlay-qa-wgi-fixture.exe"
  );
  compileObject(
    vcvars,
  ...
  const launcherObject = path.join(objectRoot, "wgi-launcher.obj");
