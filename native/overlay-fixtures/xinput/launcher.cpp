...
constexpr wchar_t kFixtureName[] = L"gamehub-overlay-qa-xinput-fixture.exe";
...
bool InitializeContainedStartup(JobStartup* contained) {
  ...
  if (contained->job == nullptr) return false;
...
int wmain(int argument_count, wchar_t** arguments) {
  if (argument_count != 7 && argument_count != 9 && argument_count != 11) {
    std::wcerr << L"usage: launcher --result <path> --scenario <name> "
                  L"--enable <mode> [--timeout-ms <100..60000>] "
                  L"[--crash-pid <path>]\n";
    return 90;
  }
  if (wcscmp(arguments[1], L"--result") != 0 ||
      wcscmp(arguments[3], L"--scenario") != 0 ||
      wcscmp(arguments[5], L"--enable") != 0) {
    return 90;
  }
  ...
  if (!ExactAnsiPath(bootstrap, &bootstrap_ansi)) return 94;

  JobStartup contained;
  if (!InitializeContainedStartup(&contained)) {
    std::wcerr << L"could not initialize kill-on-close job: " << GetLastError()
               << L"\n";
    return 95;
  }

  std::wstring command_line =
      QuoteWindowsArgument(fixture) + L" --result " +
      QuoteWindowsArgument(arguments[2]) + L" --scenario " +
      QuoteWindowsArgument(arguments[4]) + L" --enable " +
      QuoteWindowsArgument(arguments[6]);
  ...
  if (wait != WAIT_OBJECT_0) {
    ...
    CloseHandle(process.hProcess);
    if (!terminated || terminated_wait != WAIT_OBJECT_0) return 100;
