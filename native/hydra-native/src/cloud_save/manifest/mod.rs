...
pub async fn get_save_rules_for_game(
    ...
    // Console/emulator titles are resolved by GameHub's exact per-title
    // mapper. Fuzzy PC-title matching here can bind an emulated game to an
    // unrelated native game's Ludusavi entry.
    if input.shop == "launchbox" {
        return Ok(rules::build_game_save_rules(
            input.shop,
            input.object_id,
            None,
        ));
