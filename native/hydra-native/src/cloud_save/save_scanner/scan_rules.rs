...
use super::scan_path::{scan_resolved_path_with_capture_and_budget, ScanBudget};
        ...
        .map(|rule| {
            ...
            if rule.unresolved_tokens.is_empty() {
                for candidate in &rule.resolved_paths {
                    ...
                    let mut scanned_paths = match scan_resolved_path_with_capture_and_budget(
                        &candidate.path,
                        ...
                        Err(error) => {
                            if matches!(
                                error.as_str(),
                                "cloud_save_too_many_files" | "cloud_save_snapshot_too_large"
                            ) {
                                return Err(error);
                            }
                            coverage.push(UserLocationCoverage {
...
mod tests {
    ...
    use super::*;
    use crate::cloud_save::local_snapshot::guardrails::MAX_SNAPSHOT_TOTAL_SIZE_BYTES;
    ...
    #[test]
    fn applies_the_aggregate_size_budget_across_rules() {
        let temp = tempdir().unwrap();
        let first = temp.path().join("first.sav");
        let second = temp.path().join("second.sav");
        let half = MAX_SNAPSHOT_TOTAL_SIZE_BYTES / 2 + 1;
        fs::File::create(&first).unwrap().set_len(half).unwrap();
        fs::File::create(&second).unwrap().set_len(half).unwrap();
        let make_rule = |rule_id: &str, path: &Path| ResolvedCloudSaveRule {
            rule_id: rule_id.into(),
            kind: "file".into(),
            raw_path: format!("<home>/Game/{rule_id}.sav"),
            source: "ludusavi".into(),
            tags: vec!["save".into()],
            when: vec![],
            resolved_paths: vec![ResolvedCloudSavePath {
        ...
        let error = scan_rules(vec![
            make_rule("first", &first),
            make_rule("second", &second),
        ])
        .unwrap_err();

        assert_eq!(error, "cloud_save_snapshot_too_large");
    }

    #[cfg(unix)]
    #[test]
    fn production_rule_scans_never_follow_symlink_roots() {
