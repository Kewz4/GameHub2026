...
use crate::cloud_save::local_snapshot::guardrails::{
    MAX_SNAPSHOT_FILE_COUNT, MAX_SNAPSHOT_TOTAL_SIZE_BYTES,
};
...
impl ScanBudget {
    fn include_file(&mut self, path: &Path) -> Result<(), String> {
        ...
        self.file_count = self
            .file_count
            .checked_add(1)
            .ok_or_else(|| "cloud_save_too_many_files".to_string())?;
        if self.file_count > MAX_SNAPSHOT_FILE_COUNT {
            return Err("cloud_save_too_many_files".to_string());
        }
        self.total_size_bytes = self
            .total_size_bytes
            .checked_add(metadata.len())
            .ok_or_else(|| "cloud_save_snapshot_too_large".to_string())?;
        if self.total_size_bytes > MAX_SNAPSHOT_TOTAL_SIZE_BYTES {
            return Err("cloud_save_snapshot_too_large".to_string());
        }
...
fn component_matches(
    ...
    // A Windows 8.3 component (for example KENNET~1) is an exact filesystem
    // alias even though it cannot equal the directory's long display name.
    let exact_alias = cfg!(windows) && pattern.contains('~');
    if !has_pattern && (!case_sensitive || exact_alias) && exact.exists() {
    ...
    let matcher = has_pattern
        ...
        .transpose()?;
    let literal_pattern = (!has_pattern).then(|| {
        if case_sensitive {
            pattern.to_string()
        } else {
    ...
    for entry in entries {
        ...
        let matches_pattern = if let Some(matcher) = &matcher {
            ...
            if case_sensitive {
                file_name == *literal_pattern
            } else {
                file_name.to_lowercase() == *literal_pattern
            }
        } else {
            false
        };
...
fn component_traversal_matches(
    pattern: &str,
    case_sensitive: bool,
    follow_links: bool,
) -> Result<Vec<PathBuf>, String> {
...
fn glob_matches(pattern: &str, options: MatchOptions) -> Result<Vec<PathBuf>, String> {
    ...
    for expanded in expand_braces(pattern)? {
        GlobBuilder::new(&expanded)
            ...
            .build()
            .map_err(|error| format!("cloud_save_invalid_glob: {error}"))?;
        // globetter does not enumerate absolute drive-letter patterns reliably
        // on Windows when link following is disabled. Windows volumes are
        // normally case-insensitive, so use the bounded component walker there
        // while still honoring explicit case-sensitive tests. Besides fixing
        // the absolute path case, this preserves the no-symlink traversal policy.
        #[cfg(windows)]
        {
            matches.extend(component_traversal_matches(
                &expanded,
...
fn scan_directory(
        ...
        .follow_links(follow_links)
    {
        ...
        budget.include_file(entry.path())?;
...
pub(super) fn scan_resolved_path_with_capture_and_budget(
    resolved_path: &str,
    case_sensitive: bool,
    ...
    for matched in matches {
        ...
        if !follow_links {
            match std::fs::symlink_metadata(&matched) {
        ...
        if metadata.is_dir() {
            let mut scanned = scan_directory(&matched, follow_links, budget)?;
...
pub fn scan_resolved_path_with_capture(
    ...
    scan_resolved_path_with_capture_and_budget(
        resolved_path,
        case_sensitive,
        scan_root_pattern,
        capture_template,
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn rejects_a_scan_root_that_is_itself_a_symlink_when_links_are_disabled() {
        use std::os::unix::fs::symlink;
    ...
    #[test]
    fn rejects_scan_file_count_and_aggregate_size_budgets() {
        let temp = tempdir().unwrap();
        let save = temp.path().join("save.dat");
        fs::write(&save, b"save").unwrap();

        let mut count_budget = ScanBudget {
            file_count: MAX_SNAPSHOT_FILE_COUNT,
            total_size_bytes: 0,
        };
        assert_eq!(
            count_budget.include_file(&save),
            Err("cloud_save_too_many_files".to_string())
        );

        let mut size_budget = ScanBudget {
            file_count: 0,
            total_size_bytes: MAX_SNAPSHOT_TOTAL_SIZE_BYTES - 1,
        };
        assert_eq!(
            size_budget.include_file(&save),
            Err("cloud_save_snapshot_too_large".to_string())
        );
    }
}
