...
fn normalized_store(value: &str) -> String {
...
pub fn rule_is_applicable(
    ...
    conditions.is_empty()
        || conditions.iter().any(|condition| {
            ...
            let store_matches = condition.store.as_deref().map_or(true, |store| {
                normalized_store(store) == normalized_store(&context.shop)
            });
...
mod tests {
    ...
    #[test]
    fn recognizes_gamehub_store_aliases() {
        for (shop, manifest_store) in [
            ("xbox", "microsoft"),
            ("ubisoft", "uplay"),
            ("ea", "origin"),
            ("battlenet", "blizzard"),
            ...
            let mut current = context("windows", false);
            current.shop = shop.into();
            assert!(rule_is_applicable(
                &[condition(Some("windows"), Some(manifest_store))],
                &current
            ));
        }
    }
