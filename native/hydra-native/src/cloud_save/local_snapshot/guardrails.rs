...
pub const MAX_SNAPSHOT_FILE_COUNT: usize = 20_000;
...
pub fn validate_built_files(files: &[BuiltLocalSaveFile]) -> Result<u64, LocalSnapshotGuardError> {
    ...
    for file in files {
        if !file.size_bytes.is_finite() || file.size_bytes < 0.0 || file.size_bytes.fract() != 0.0 {
            return Err(LocalSnapshotGuardError::HashSizeMismatch);
        }
        if file.size_bytes > MAX_SNAPSHOT_TOTAL_SIZE_BYTES as f64 {
            return Err(LocalSnapshotGuardError::SnapshotTooLarge);
        }
...
mod tests {
    ...
    #[test]
    fn best_effort_discovery_rejects_files_above_the_upload_limit() {
    ...
    #[test]
    fn built_snapshot_rejects_files_above_the_upload_limit() {
        ...
        assert_eq!(
            validate_built_files(&[file]),
            Err(LocalSnapshotGuardError::SnapshotTooLarge)
        );
    }

    #[test]
    fn best_effort_and_built_validation_enforce_file_count() {
        let too_many = (0..=MAX_SNAPSHOT_FILE_COUNT)
            .map(|index| discovered("variant", &format!("save-{index}.dat")))
            .collect::<Vec<_>>();
        assert!(matches!(
            prepare_snapshot_files_best_effort(&too_many),
            Err(LocalSnapshotGuardError::TooManyFiles)
        ));

        let built = (0..=MAX_SNAPSHOT_FILE_COUNT)
            .map(|index| BuiltLocalSaveFile {
                variant_id: "variant".into(),
                rule_id: "rule".into(),
                raw_path: "<home>/game".into(),
                relative_path: format!("save-{index}.dat"),
                absolute_path: format!("/game/save-{index}.dat"),
