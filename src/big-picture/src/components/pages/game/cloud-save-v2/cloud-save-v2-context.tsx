...
import { BigPictureCloudSaveFileDetailsModal } from "./cloud-save-file-details-modal";
...
export function BigPictureCloudSaveProvider({
  ...
  const {
    ...
    objectId,
    shop,
    enabled: canCheckCloudSaves && isFileDetailsVisible,
  });
  ...
  useEffect(() => {
    if (searchParams.get("openCloudSaveConflict") !== "1") {
      return;
    }
  ...
  const handleAddCustomPath = async (selectedPath: string) => {
    const requestedGame = gameKey;
    let wasAdded = false;
    setIsSyncing(true);
    setHasSyncError(false);
    ...
    try {
      const result = await globalThis.window.electron.selectCloudSaveCustomPath(
        objectId,
        shop,
        selectedPath
      );
      if (result.canceled || !result.customPath) return;

      wasAdded = true;
      await globalThis.window.electron.syncGameCloudSave(
        objectId,
        shop,
        (nextProgress) => {
          if (activeGameKey.current === requestedGame) {
  ...
  const handleRemoveCustomPath = async (rawPath: string) => {
    const requestedGame = gameKey;
    setIsSyncing(true);
    setProgress(null);
    try {
      await globalThis.window.electron.removeCloudSaveCustomPath(
        objectId,
        shop,
        rawPath,
        (nextProgress) => {
      ...
      showErrorToast(t("cloud_save_v2_custom_path_remove_error_title"), {
        message: t("cloud_save_v2_custom_path_remove_error_description"),
      });
      throw error;
    } finally {
      if (activeGameKey.current === requestedGame) {
  ...
  const handleRequestCustomPathRebind = async (rawPath: string) => {
    const approval =
      await globalThis.window.electron.createCloudSaveCustomPathRebindApproval(
        objectId,
        shop,
        rawPath
      );
  ...
  const handleDeleteCloudSave = async () => {
    setIsSyncing(true);
    try {
      await globalThis.window.electron.deleteGameCloudSaveData(objectId, shop);
      showSuccessToast(t("cloud_save_v2_delete_success"));
    } catch (error) {
      showErrorToast(t("cloud_save_v2_delete_error_title"), {
        message: t("cloud_save_v2_delete_error_description"),
      });
      throw error;
    } finally {
      await refresh();
  ...
  return (
    <bigPictureCloudSaveContext.Provider value={value}>
      ...
      <BigPictureCloudSaveFileDetailsModal
        ...
        onRebindCustomPath={handleRequestCustomPathRebind}
