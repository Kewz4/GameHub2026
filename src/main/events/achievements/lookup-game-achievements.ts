...
const lookupGameAchievements = async (
  ...
  if (!slug) {
    achievementsLogger.warn(
      `[Exophase lookup] no Exophase slug for shop "${shop}"`
    );
    return {
      found: false,
      achievementCount: 0,
      unlockedCount: 0,
      error: `No Exophase support for shop "${shop}".`,
    };
  }

  achievementsLogger.log(
    `[Exophase lookup] searching "${game.title}" on "${slug}"`
  );
  ...
  try {
    ...
    achievementsLogger.log(
      `[Exophase lookup] "${game.title}" → ${candidates.length} candidates`
    );

    const match = findBestMatch(game.title, candidates, slug);
    if (!match) {
      achievementsLogger.log(
        `[Exophase lookup] no match for "${game.title}" on "${slug}"`
      );
      WindowManager.sendToAppWindows("on-exophase-lookup-progress", {
        status: "not_found",
      ...
      return {
        ...
        achievementCount: 0,
        unlockedCount: 0,
        error: `Not found on Exophase for platform ${slug}.`,
      };
    }

    const url = awardsUrlFor(match);
    achievementsLogger.log(
      `[Exophase lookup] match: "${match.title}" → ${url}`
    );
    ...
    achievementsLogger.warn(
      `[Exophase lookup] failed for "${game.title}"`,
      err
    );
