...
const getLibrary = async (): Promise<LibraryGame[]> => {
  return Promise.all([
    ...
    playnitePlaytimeCacheSublevel.iterator().all(),
  ]).then(([results, playniteCacheEntries]) => {
    const playniteRecords =
      playniteCacheEntries as PlaynitePlaytimeCacheRecord[];
    const activeResults = results.filter(
      ([_key, game]) => game.isDeleted === false
    );
    const playniteAssignments = assignPlayniteCacheEntriesToGames(
      playniteRecords,
      activeResults.map(([key, game]) => ({
        key,
        shop: game.shop,
    ...
    return Promise.all(
      activeResults.map(async ([key, storedGame]) => {
        let game = storedGame;
        const assignment = playniteAssignments.get(key);
        const cachedPlaytime = assignment?.selected ?? null;
        if (cachedPlaytime) {
          const decision = decidePlaynitePlaytimeImport(
            game.playTimeInMilliseconds,
            cachedPlaytime[1].playTimeInMilliseconds
          );
          ...
          await Promise.all(
            (assignment?.consumedKeys ?? []).map((cacheKey) =>
              playnitePlaytimeCacheSublevel.del(cacheKey)
            )
          );
        }
        ...
        if (
          ...
          if (system) {
            meta = await getGameHubMeta(system, game.title).catch(() => null);
          }
        }
