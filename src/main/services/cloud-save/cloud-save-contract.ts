...
const GAMEHUB_CLOUD_SAVE_SHOPS = new Set<GameShop>([
  "steam",
  "epic",
  "gog",
  "battlenet",
...
const isGameHubCloudSaveShop = (value: unknown): value is GameShop =>
  typeof value === "string" && GAMEHUB_CLOUD_SAVE_SHOPS.has(value as GameShop);
...
export const validateRestoreManifest = (
  ...
  if (
    ...
    !Number.isSafeInteger(snapshot.version) ||
    snapshot.version < 1 ||
    !isGameHubCloudSaveShop(snapshot.shop) ||
