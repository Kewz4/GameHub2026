...
describe("Cloud Save snapshot retry policy", () => {
  ...
  it("recalculates the full flow once for 409 and never for 429", () => {
    ...
    const r2Conflict = new Error("cloud_save_remote_head_conflict") as Error & {
      code: string;
    };
    r2Conflict.code = "cloud_save_remote_head_conflict";
    assert.equal(shouldRetryCloudSaveConflict(r2Conflict, 0), true);
    assert.equal(shouldRetryCloudSaveConflict(r2Conflict, 1), false);
  });
});
