...
import {
  ...
  unionCloudSaveUnresolvedRemoteEntryIds,
} from "./cloud-save-v2-file-details";
// @ts-ignore The Node ESM test runner requires the source extension.
...
describe("cloud save V2 file details", () => {
  it("unions unresolved bindings carried by the anchor and current merge", () => {
    const remoteFiles = [file(firstVariantId, "a"), file(secondVariantId, "b")];
    const unresolvedRemoteEntryIds = unionCloudSaveUnresolvedRemoteEntryIds(
      [cloudSaveFileKey(remoteFiles[0]), "shared"],
      [cloudSaveFileKey(remoteFiles[1]), "shared"]
    );
    ...
    assert.equal(new Set(unresolvedRemoteEntryIds).size, 3);
    assert.equal(details.unresolvedRemoteVariantCount, 2);
  });
