...
} from "./automatic-sync-observation.js";
...
describe("automatic cloud save observation", () => {
  ...
  it("does not suppress a sync when no observation was recorded", () => {
    const result = beginAutomaticSyncObservation(
      "10",
      "steam",
      "game-page-open",
      1
    ...
    assert.equal(result.observationKey, null);
  });

  it("ignores an A completion that arrives after account-session invalidation", () => {
    recordLatestCloudSaveObservation("10", "steam", "same", "account-a");
    const attemptA = beginAutomaticSyncObservation(
      "10",
      "steam",
      "game-page-open",
      1,
    ...
    finishAutomaticSyncObservation(
      ...
      "steam",
      "game-page-open",
      attemptA.observationKey,
      "settled",
      2,
      "account-a",
      attemptA.sessionGeneration
    );
