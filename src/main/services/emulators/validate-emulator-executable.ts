...
import { findMacAppBundleRoot, resolveMacAppBundleExecutable } from "./macos-app-bundle";
...
const NON_EXECUTABLE_EXTENSIONS = new Set([
  ...
  ".iso", ".bin", ".cue", ".chd", ".pkg", ".cso",
  ".json", ".xml", ".ini", ".cfg", ".log", ".html", ".htm",
]);
...
export const isValidEmulatorExecutable = (executablePath: string): boolean => {
  ...
  if (appBundlePath) {
    return ext === ".app" && resolveMacAppBundleExecutable(appBundlePath) !== null;
  }
  try {
