...
export const resolveMacAppBundleExecutable = (appBundlePath: string): string | null => {
  if (process.platform !== "darwin") return null;
  const execDir = path.join(appBundlePath, "Contents", "MacOS");
  return existsSync(execDir) ? execDir : null;
};

export const resolveEmulatorExecutableTarget = (executablePath: string): string | null => {
