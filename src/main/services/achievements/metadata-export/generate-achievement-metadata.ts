...
const writeAchievementsFile = async (
  ...
  try {
    ...
    await fs.promises.link(temporaryFilePath, filePath);
    await fs.promises.rm(temporaryFilePath, { force: true });
  } catch (error) {
...
export const generateAchievementMetadata = async (
  ...
  if (game.shop !== "steam" || game.libraryOrigin === "sync") return null;
