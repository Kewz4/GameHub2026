...
import capturePagePath from "@resources/game-screenshot-capture.html?asset";
...
const destroyCaptureWindow = (captureWindow: BrowserWindow) => {
  ...
  void captureWindow.webContents
    .executeJavaScript(
      "globalThis.__gamehubSouvenirStream?.getTracks().forEach((track) => track.stop())"
    )
    .catch(() => {})
    .finally(() => {
  ...
  try {
    ...
    await captureWindow.webContents.executeJavaScript(`
      (async () => {
        ...
        globalThis.__gamehubSouvenirStream = stream;
        globalThis.__gamehubSouvenirVideo = video;
      })()
    `);
...
const captureFrame = async (captureWindow: BrowserWindow) => {
  const result: unknown = await captureWindow.webContents.executeJavaScript(`
    (async () => {
      const video = globalThis.__gamehubSouvenirVideo;
      const track = globalThis.__gamehubSouvenirStream?.getVideoTracks()[0];
