// SteamAutoCrack integration smoke test — runs inside Electron so the
...
// imported crack chunk transitively loads it. Mirror the registration first.
protocol.registerSchemesAsPrivileged([
  { scheme: "local", privileges: { secure: true, supportFetchAPI: true, bypassCSP: true, stream: true } },
  { scheme: "controller-tester", privileges: { standard: true, secure: true, supportFetchAPI: true } },
]);

const run = async () => {
  const service = await servicePromise;

  console.log("=== 1. crack tool available? ===");
  console.log(service.isCrackToolAvailable());

  console.log("\n=== 2. detectCrackStatus on real game folders ===");
  const targets = [
  ...
  for (const dir of targets) {
    const detection = service.detectCrackStatus(dir);
    console.log(
  ...
  console.log("\n=== 3. ensureCrackConfig ===");
  const configPath = await service.ensureCrackConfig();
  console.log("config:", configPath);
  if (configPath) {
    const config = JSON.parse(fs.readFileSync(configPath, "utf8"));
    console.log("SteamWebAPIKey set:", !!config.EMUGameInfoConfigs?.SteamWebAPIKey);
    console.log(
  ...
  console.log("\n=== 5. crackGame on scratch folder (end-to-end) ===");
  const testDir = path.join(
    app.getPath("temp"),
    `gamehub-crack-test-${Date.now()}`
  );
  ...
  const result = await service.crackGame(testDir, "2651280");
  ...
  console.log(
    ...
    fs.existsSync(path.join(testDir, "steam_settings")) ||
      (fs.existsSync(path.join(testDir, "steam_api64.dll")) &&
        fs.statSync(path.join(testDir, "steam_api64.dll")).size > 20 * 1024 * 1024)
  );
...
const chunk = fs
  ...
  .find((f) => f.startsWith("steam-auto-crack-") && f.endsWith(".js"));
...
const servicePromise = chunk
  ...
  : Promise.reject(new Error("steam-auto-crack chunk not found in out/main"));
